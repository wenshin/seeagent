...
let deviceRulesObj = require('./rules/device');
let appQueryRulesObj = require('./rules/query');
let appUserAgentRulesObj = require('./rules/agent');
...
function toUAElement(family, versions) {
...
function parseAppOfUserAgent(httpUserAgent) {
  let rulesObj = appUserAgentRulesObj;
  for (const key of Object.keys(rulesObj)) {
    let versions = parseRules(httpUserAgent, rulesObj[key]);
    if (versions) return toUAElement(key, versions);
  }
}
 ...
 * @param  {Object} device         useragent 库解析的设备信息
 * @return {String}                设备类型
 */
function parseDeviceOfUserAgent(httpUserAgent, device) {
  let info = {};
  let newDevice = device;
  if (device.family === 'Other') {
    for (const key of Object.keys(deviceRulesObj)) {
      let {rules} = deviceRulesObj[key];
      let versions = parseRules(httpUserAgent, rules);
      if (versions) {
        newDevice = toUAElement(key, versions);
        info = deviceRulesObj[key].info || {};
        break;
      }
    }
  }

  if (!info.type) info.type = uaDeviceType(httpUserAgent);

  return Object.assign({}, newDevice, info);
}
...
function parseUserAgent(httpUserAgent, jsUserAgent) {
  ...
  let app = parseAppOfUserAgent(httpUserAgent);
...
function parseAppOfQuery(query) {
  if (!query) return;
  for (const key of Object.keys(appQueryRulesObj)) {
    let rules = appQueryRulesObj[key];
...
SeeAgent.extendFingerprints = function extendFingerprints(fingerprintsObj) {
  Object.assign(appUserAgentRulesObj, fingerprintsObj);
};
...
SeeAgent.extendQueryRules = function extendQueryRules(rulesObj) {
  Object.assign(appQueryRulesObj, rulesObj);
};
