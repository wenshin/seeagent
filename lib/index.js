...
function parseAppOfUserAgent(httpUserAgent, appElement) {
  ...
  let newAppElement = Object.assign({}, appElement);
  newAppElement.isWebView = /webview/i.test(newAppElement.family);
  for (const config of configs) {
    let info = parseCustomized(httpUserAgent, config);
    if (info) {
      Object.assign(newAppElement, info);
      break;
    }
  }
 ...
 * @param  {Array} ruleConfigs 指纹定义数组
 *     eg.
 *     ```
 *       [
...
SeeAgent.extendFingerprints = function extendFingerprints(ruleConfigs) {
  appRuleConfigs.useragent = appRuleConfigs.useragent.concat(ruleConfigs);
};

/**
 * 扩展自定义的 HTTP 请求 query    参数识别规则
 * @param  {Array} ruleConfigs query 识别规则数组
 *     eg.
 *     ```
 *       [
 *         {info: {vendor: 'App1'}, rules: [/App123/i, /App234/i]},
...
SeeAgent.extendQueryRules = function extendQueryRules(ruleConfigs) {
  appRuleConfigs.query = appRuleConfigs.query.concat(ruleConfigs);
};
