...
let appRulesObj = require('./rules/app');
...
function parseAppOfUserAgent(httpUserAgent) {
  let rulesObj = appRulesObj.useragent;
...
function parseAppOfQuery(query) {
  if (!query) return;
  for (const key of Object.keys(appRulesObj.query)) {
    let rules = appRulesObj.query[key];
...
SeeAgent.extendFingerprints = function extendFingerprints(fingerprintsObj) {
  Object.assign(appRulesObj.useragent, fingerprintsObj);
};
...
SeeAgent.extendQueryRules = function extendQueryRules(rulesObj) {
  Object.assign(appRulesObj.query, rulesObj);
};
